...
//                }
        ...
        foreach ($conds_ex as $section_id => $section) {
            ...
            foreach ($section as $item_id => $cond) {
                if (!isset($config[$section_id]['items'][$item_id])) {
                    if ($section_id === 'contact_info' && $cm->fieldExists($item_id)) {
                        continue;
                    }
    ...
    /**
     * SQL expression for wa_contact column search (mirrors waContactsCollection::getExpression).
     *
     * @param waModel $m
     * @param string $op
     * @param string $value
     * @return string
     */
    protected static function getWaContactFieldExpression($m, $op, $value)
    {
        switch ($op) {
            case '>':
            case '>=':
            ...
            case '<=':
            case '!=':
                return " {$op} '" . $m->escape($value) . "'";
            case '^=':
                return " LIKE '" . $m->escape($value, 'like') . "%'";
            case '$=':
                return " LIKE '%" . $m->escape($value, 'like') . "'";
            case '*=':
                return " LIKE '%" . $m->escape($value, 'like') . "%'";
        ...
        if (isset($conds['contact_info'])) {
            ...
            foreach ($conds['contact_info'] as $item_id => $val_item) {
                if ($item_id === 'name') {
                    ...
                    } elseif (isset($val_item['val']) && $m->fieldExists($item_id)) {
                        $collection->addWhere('c.' . $item_id . self::getWaContactFieldExpression($m, $val_item['op'], $val_item['val']));
                        $highlight_terms[] = $val_item['val'];
                        unset($conds['contact_info'][$item_id]);
                    }
                }
